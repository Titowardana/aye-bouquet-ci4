...
<div class="flex items-center gap-2 text-sm text-on-surface-variant mb-6">
    ...
    <span class="text-on-surface font-semibold">Edit Produk</span>
</div>

<!-- Flash Messages -->
<?php if (!empty($validation->getErrors())): ?>
<div class="mb-6 p-4 rounded-xl bg-[#FCE8E6] border border-[#FAD2CF] text-[#C5221F] text-sm">
...
<form action="<?= base_url('admin/produk/update/' . $product['id']) ?>" method="post" enctype="multipart/form-data" class="admin-enter">
    <?= csrf_field() ?>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-surface-container-lowest dark:bg-on-background rounded-2xl p-6 soft-shadow border border-outline-variant/20">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <div class="relative group" id="image-box-<?= $img['id'] ?>">
                        <img src="<?= base_url('uploads/products/' . $img['image']) ?>" alt="Product image" class="w-full h-32 object-cover rounded-xl border border-outline-variant/30">
                        <?php if ($img['is_primary']): ?>
                        <span class="absolute top-2 left-2 bg-primary text-on-primary text-[10px] font-bold px-2 py-1 rounded-full shadow-sm">Utama</span>
                        <?php endif; ?>
                        ...
                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center gap-2">
                            <button type="button" onclick="deleteImage(<?= $img['id'] ?>)" class="bg-error text-on-error p-2 rounded-full hover:scale-110 transition-transform" title="Hapus Foto">
            ...
            <div class="bg-surface-container-lowest dark:bg-on-background rounded-2xl p-6 soft-shadow border border-outline-variant/20">
                ...
                <div class="space-y-1.5">
                    <label for="product-images-input" class="border-2 border-dashed border-outline-variant/60 rounded-2xl p-8 text-center hover:border-primary/40 transition-colors cursor-pointer flex flex-col items-center gap-3 group">
                        ...
                        <div>
                            <span class="text-sm font-bold text-primary dark:text-primary-fixed-dim">Klik untuk unggah foto tambahan</span>
            ...
            <div class="bg-surface-container-lowest dark:bg-on-background rounded-2xl p-6 soft-shadow border border-outline-variant/20">
                ...
                <div class="flex flex-col gap-3 mt-6 pt-5 border-t border-outline-variant/20">
                    <button type="submit" class="bg-primary text-on-primary hover:bg-on-primary-fixed-variant w-full py-3 rounded-full text-sm font-bold shadow-sm transition-all flex items-center justify-center gap-2">
                        <span class="material-symbols-outlined text-lg">save</span> Perbarui Produk
                    </button>
        ...
        Array.from(input.files).forEach((file, index) => {
            ...
            reader.onload = (e) => {
                ...
                div.innerHTML = `
                    ...
                    <span class="absolute top-1 left-1 bg-secondary text-on-secondary text-[8px] font-bold px-1.5 py-0.5 rounded-full">Baru</span>
                `;
    ...
    function deleteImage(imageId) {
        openAdminConfirm({
            ...
            onConfirm: function() {
                ...
                .then(data => {
                    if (data.success) {
                        document.getElementById('image-box-' + imageId).remove();
                    } else {
