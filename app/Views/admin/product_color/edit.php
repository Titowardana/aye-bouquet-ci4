...
<div class="flex items-center gap-2 text-sm text-on-surface-variant dark:text-white/60 mb-6">
    ...
    <span class="material-symbols-outlined text-base">chevron_right</span>
    <span class="text-on-surface dark:text-white font-medium">Edit Warna</span>
</div>
...
<div class="max-w-2xl mx-auto">
    <form action="<?= base_url('admin/product-colors/update/' . $color['id']) ?>" method="post">
        <?= csrf_field() ?>
        ...
        <div class="bg-surface-container-lowest dark:bg-white/[0.03] rounded-2xl p-6 soft-shadow border border-outline-variant/20 dark:border-white/5 space-y-6">
            ...
            <div>
                ...
                <?php
                $nameErr = (!empty($validation) && $validation->hasError('name')) ? $validation->getError('name') : (session()->getFlashdata('errors')['name'] ?? '');
                if ($nameErr):
                ?>
                <p class="mt-1.5 text-xs text-error"><?= esc($nameErr) ?></p>
                <?php endif; ?>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="text" id="hex_code" name="hex_code" value="<?= old('hex_code', $color['hex_code'] ?? '') ?>"
                    ...
                    <div id="color-preview" class="w-10 h-10 rounded-full border border-outline-variant/30 dark:border-white/10 flex-shrink-0" style="background-color: <?= old('hex_code', $color['hex_code'] ?? '#f0f0f0') ?>"></div>
                </div>
                <?php
                $hexErr = (!empty($validation) && $validation->hasError('hex_code')) ? $validation->getError('hex_code') : (session()->getFlashdata('errors')['hex_code'] ?? '');
                if ($hexErr):
                ?>
                <p class="mt-1.5 text-xs text-error"><?= esc($hexErr) ?></p>
                <?php endif; ?>
            ...
            <div>
                ...
                <input type="number" id="sort_order" name="sort_order" value="<?= old('sort_order', $color['sort_order'] ?? 0) ?>"
                       ...
                       placeholder="0" min="0">
                <?php
                $orderErr = (!empty($validation) && $validation->hasError('sort_order')) ? $validation->getError('sort_order') : (session()->getFlashdata('errors')['sort_order'] ?? '');
                if ($orderErr):
                ?>
                <p class="mt-1.5 text-xs text-error"><?= esc($orderErr) ?></p>
                <?php endif; ?>
            </div>

            <div class="flex items-center gap-3">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" id="is_active" name="is_active" value="1" <?= old('is_active', $color['is_active']) ? 'checked' : '' ?>
