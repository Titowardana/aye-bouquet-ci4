...
<div class="mb-8 flex items-center justify-between">
    <div>
        <h2 class="font-headline-lg text-headline-lg text-on-surface mb-2">Tambah Kategori</h2>
        <p class="font-body-md text-body-md text-on-surface-variant max-w-2xl">Lengkapi data kategori produk yang akan ditampilkan pada katalog. Gunakan gambar resolusi tinggi untuk hasil terbaik.</p>
    </div>
            ...
            <div class="p-6 md:p-8 space-y-8">
                ...
                <div>
                    ...
                    <div class="relative border-2 border-dashed border-outline-variant dark:border-white/15 hover:border-primary transition-colors bg-surface-bright dark:bg-white/5 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer group min-h-[200px]" id="uploadContainer">
                        ...
                        <div id="uploadPrompt" class="flex flex-col items-center">
                            <div class="w-16 h-16 rounded-full bg-primary-container dark:bg-primary/25 text-primary dark:text-primary-fixed-dim flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                        ...
                        <div id="imagePreviewContainer" class="hidden absolute inset-0 w-full h-full bg-surface-bright dark:bg-white/5 rounded-xl overflow-hidden flex items-center justify-center">
                            ...
                            <div class="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                                <span class="bg-surface-container-lowest text-on-surface px-4 py-2 rounded-lg font-label-md flex items-center gap-2">
                                    <span class="material-symbols-outlined text-sm">edit</span> Ganti Foto
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input name="name" id="categoryName" value="<?= old('name') ?>" class="w-full bg-surface-container-lowest border border-outline-variant rounded-xl px-4 py-3 font-body-md text-body-md text-on-surface shadow-sm outline-none transition-all focus:border-primary focus:ring-1 focus:ring-primary placeholder:text-outline-variant" placeholder="Cth: Buket Bunga" type="text" required>
                    </div>

                    <!-- Input: Slug Kategori -->
                    <div>
                        <label class="block font-label-md text-label-md text-on-surface dark:text-white mb-2" for="categorySlug">Slug Kategori</label>
                        <input name="slug" id="categorySlug" value="<?= old('slug') ?>" class="w-full bg-surface-container-lowest border border-outline-variant rounded-xl px-4 py-3 font-body-md text-body-md text-on-surface shadow-sm outline-none transition-all focus:border-primary focus:ring-1 focus:ring-primary placeholder:text-outline-variant" placeholder="Cth: buket-bunga" type="text">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-outline-variant/40">
                        ...
                        <div class="flex items-center gap-4">
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <div class="relative flex items-center justify-center w-5 h-5">
                                    <input name="is_active" type="radio" value="1" class="peer sr-only" <?= old('is_active', '1') == '1' ? 'checked' : '' ?>>
                            ...
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <div class="relative flex items-center justify-center w-5 h-5">
                                    <input name="is_active" type="radio" value="0" class="peer sr-only" <?= old('is_active') == '0' ? 'checked' : '' ?>>
                                    <div class="w-5 h-5 rounded-full border-2 border-outline-variant peer-checked:border-primary transition-colors"></div>
                                    <div class="absolute w-2.5 h-2.5 rounded-full bg-primary scale-0 peer-checked:scale-100 transition-transform duration-200"></div>
                                </div>
                    ...
                    <div>
                        ...
                        <input name="sort_order" id="categoryOrder" value="<?= old('sort_order', '1') ?>" class="w-24 bg-surface-container-lowest border border-outline-variant rounded-xl px-4 py-2 font-body-md text-body-md text-on-surface shadow-sm outline-none transition-all focus:border-primary focus:ring-1 focus:ring-primary" type="number" min="1">
                    </div>
                </div>
            </div>
        ...
        <div class="xl:col-span-4 sticky top-[100px]">
            ...
            <div class="bg-surface-container-lowest rounded-[1rem] overflow-hidden shadow-sm border border-outline-variant/20 hover:shadow-md transition-all duration-300 group">
                ...
                <div class="relative h-48 w-full bg-surface-container-high overflow-hidden flex items-center justify-center">
                    <img id="livePreviewImg" src="" alt="Preview" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 hidden">
                    <span id="livePreviewIcon" class="material-symbols-outlined text-6xl text-outline-variant/50">category</span>
                ...
                <div class="p-6">
                    ...
                    <h4 class="font-headline-md text-xl font-bold text-on-surface mb-2" id="livePreviewName">Nama Kategori</h4>
                    <p class="font-body-md text-sm text-on-surface-variant line-clamp-2 mb-6" id="livePreviewDesc">Deskripsi singkat kategori akan tampil di sini.</p>
                    
                    <button type="button" class="w-full font-label-md text-sm font-semibold border border-outline-variant text-on-surface py-2.5 rounded-full flex justify-center items-center gap-2 group-hover:border-primary group-hover:text-primary transition-colors cursor-default">
                        Lihat Produk
