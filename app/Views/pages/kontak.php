...
?>
<main class="max-w-[1280px] mx-auto px-container-padding-mobile md:px-container-padding-desktop py-section-gap space-y-section-gap">
    <section class="text-center max-w-3xl mx-auto space-y-6 animate-on-scroll">
        <h1 class="font-display-lg text-4xl md:text-5xl font-bold text-primary dark:text-primary-fixed-dim">Kontak & Lokasi</h1>
        <p class="font-body-lg text-lg text-on-surface-variant dark:text-white/70 leading-relaxed">Hubungi kami untuk konsultasi, pemesanan, custom order, informasi stok, dan pengiriman.</p>
    </section>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start animate-on-scroll">
        <div class="space-y-8 order-1">
            <div class="bg-surface-container-lowest dark:bg-[#262024] p-8 rounded-xl shadow-level-1 border border-surface-container-low dark:border-white/10 card-hover">
                <div class="flex items-center space-x-4 mb-6">
                    <div class="w-12 h-12 bg-primary-container dark:bg-primary/25 text-primary dark:text-primary-fixed-dim rounded-full flex items-center justify-center"><span class="material-symbols-outlined">chat</span></div>
                ...
                <a href="https://wa.me/<?= esc($waNumber) ?>?text=<?= rawurlencode('Halo kak, saya ingin bertanya tentang produk Aye Bouquet.') ?>" target="_blank" class="w-full bg-primary hover:bg-surface-tint text-on-primary font-label-md text-sm font-bold py-4 rounded-full transition-colors flex justify-center items-center space-x-2">
            ...
            <div class="bg-surface-container-lowest dark:bg-[#262024] p-8 rounded-xl shadow-level-1 border border-surface-container-low dark:border-white/10 card-hover">
                ...
                <form class="space-y-6" id="contactForm">
                    <div><label class="block font-label-md text-sm font-bold text-on-surface-variant dark:text-white/80 mb-2" for="name">Nama Lengkap</label><input class="w-full bg-surface-bright dark:bg-white/5 border border-outline-variant dark:border-white/15 rounded-lg px-4 py-3 text-on-surface dark:text-white focus:border-primary outline-none placeholder:text-on-surface-variant/50 dark:placeholder:text-white/40" id="name" placeholder="Masukkan nama Anda" type="text" required></div>
                    <div><label class="block font-label-md text-sm font-bold text-on-surface-variant dark:text-white/80 mb-2" for="phone">Nomor Telepon / WhatsApp</label><input class="w-full bg-surface-bright dark:bg-white/5 border border-outline-variant dark:border-white/15 rounded-lg px-4 py-3 text-on-surface dark:text-white focus:border-primary outline-none placeholder:text-on-surface-variant/50 dark:placeholder:text-white/40" id="phone" placeholder="08..." type="tel" required></div>
                    <div><label class="block font-label-md text-sm font-bold text-on-surface-variant dark:text-white/80 mb-2" for="type">Jenis Kebutuhan</label><select class="w-full bg-surface-bright dark:bg-[#211b1f] border border-outline-variant dark:border-white/15 rounded-lg px-4 py-3 text-on-surface dark:text-white focus:border-primary outline-none" id="type" required><option value="Tanya Produk">Tanya Produk</option><option value="Custom Order">Custom Order</option><option value="Tanya Pengiriman">Tanya Pengiriman</option><option value="Tanya Pembayaran">Tanya Pembayaran</option><option value="Tanya Ketersediaan Produk">Tanya Ketersediaan Produk</option></select></div>
                    <div><label class="block font-label-md text-sm font-bold text-on-surface-variant dark:text-white/80 mb-2" for="message">Pesan</label><textarea class="w-full bg-surface-bright dark:bg-white/5 border border-outline-variant dark:border-white/15 rounded-lg px-4 py-3 text-on-surface dark:text-white focus:border-primary outline-none resize-none placeholder:text-on-surface-variant/50 dark:placeholder:text-white/40" id="message" placeholder="Tuliskan pertanyaan atau detail kebutuhan Anda..." rows="4" required></textarea></div>
                    <button class="w-full bg-primary-container text-on-primary-container hover:bg-primary-fixed font-label-md text-sm font-bold py-4 rounded-full transition-colors flex justify-center items-center space-x-2" type="button" onclick="sendContactToWhatsapp()"><span class="material-symbols-outlined text-[20px]">send</span><span>Kirim Pertanyaan via WhatsApp</span></button>
                </form>
            </div>
        </div>
        ...
        <div class="space-y-8 order-2">
            <div>
                ...
                <div class="bg-surface-container-lowest dark:bg-[#262024] rounded-xl overflow-hidden shadow-level-1 border border-surface-container-low dark:border-white/10 card-hover">
                    <div class="h-[300px] w-full bg-surface-variant relative overflow-hidden">
                        <?php if ($mapsEmbed !== ''): ?>
            ...
            <div class="bg-surface-container-lowest dark:bg-[#262024] p-8 rounded-xl shadow-level-1 border border-surface-container-low dark:border-white/10 card-hover">
                ...
                <ul class="space-y-4 font-body-md text-on-surface-variant dark:text-white/70">
                    <li class="flex justify-between items-center pb-4 border-b border-surface-container-high dark:border-white/10 gap-4"><span class="font-bold text-on-surface dark:text-white">Jam Buka</span><span class="text-right"><?= esc($contacts['working_hours'] ?? '-') ?></span></li>
                    <li class="flex justify-between items-center pb-4 border-b border-surface-container-high dark:border-white/10 gap-4"><span class="font-bold text-on-surface dark:text-white">Waktu Pickup</span><span class="text-right"><?= esc($contacts['pickup_hours'] ?? '-') ?></span></li>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-surface-container-lowest dark:bg-[#262024] p-6 rounded-xl shadow-level-1 border border-surface-container-low dark:border-white/10 card-hover">
                ...
                <div class="bg-surface-container-lowest dark:bg-[#262024] p-6 rounded-xl shadow-level-1 border border-surface-container-low dark:border-white/10 card-hover">
                    ...
                    <div class="space-y-3 text-on-surface-variant dark:text-white/70"><div class="flex items-center space-x-3"><span class="material-symbols-outlined">account_balance</span><span>Bank Transfer</span></div><div class="flex items-center space-x-3"><span class="material-symbols-outlined">qr_code</span><span>QRIS</span></div><p class="text-xs italic bg-surface-container-low dark:bg-white/5 rounded-lg p-3 dark:text-white/60">Pembayaran dilakukan setelah pesanan dikonfirmasi admin melalui WhatsApp. Website tidak memakai payment gateway.</p></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <section id="faq" class="max-w-4xl mx-auto space-y-6 animate-on-scroll">
        ...
        <div class="space-y-4">
            <?php foreach ($faqs as $faq): ?>
                <details class="faq-item bg-surface-container-lowest dark:bg-[#262024] border border-surface-container-low dark:border-white/10 rounded-xl p-5"><summary class="font-bold text-on-surface dark:text-white"><?= esc($faq['question']) ?></summary><p class="text-on-surface-variant dark:text-white/70 leading-relaxed"><?= nl2br(esc($faq['answer'])) ?></p></details>
            <?php endforeach; ?>
        </div>
    </section>
    <?php endif; ?>
